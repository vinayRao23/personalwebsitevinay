const timelineData: Array<{
  name: string;
  description: string;
  date: string;
  icon: string;
}> = [
  {
    name: "Started Coding",
    description: `print("Hello World"); The foundation to everything...`,
    date: "Nov. 2019",
    icon: "fa fa-terminal",
  },
  {
    name: "Raspberry Pi Hackathon Finalist",
    description: `Placed 4th place in a Raspberry Pi hackathon, where I built a game with the software.`,
    date: "Feb 2020",
    icon: "fa fa-trophy",
  },
  {
    name: "Exploring Data Science",
    description: `Using popular Python libraries such as NumPy, Pandas, and more, to create awesome charts, and data tables with code.`,
    date: "Summer 2020",
    icon: "fa fa-database",
  },
  {
    name: "Exploring Game Dev In Python",
    description: `I started my exploration into game dev with Python. Using libraries such as Pygame, I started visualizing what it was like to be a game dev.`,
    date: "Aug. 2020",
    icon: "fa fa-gamepad",
  },
  {
    name: "The Creation of Opera",
    description: `One of my earliest projects with Python. A command line voice assistant with the power to do thing other assistants like Siri could do.`,
    date: "Aug. 2020",
    icon: "fa fa-microphone",
  },
  {
    name: "Started Learning Javascript",
    description: `30 mins every day, with my brother and dad.`,
    date: "Sept. 2020",
    icon: "fa fa-code",
  },
  {
    name: "Started Learning HTML/CSS",
    description: `30 mins every day, with my brother and dad.`,
    date: "Sept. 2020",
    icon: "fa fa-html5",
  },
  {
    name: "The Start of My Web Development Journey",
    description: `Embarked on a new journey, a path that I soon admire.`,
    date: "Oct. 2020",
    icon: "fa fa-globe",
  },
  {
    name: "Entered The World Of React.JS",
    description: `The best javascript framework. No doubt about it.`,
    date: "Nov. 2020",
    icon: "fa fa-code",
  },
  {
    name: "The Creation of OnTrack",
    description: `A productivity app built with ReactJS to test my knowledge of React, and to help stay on track with assignments.`,
    date: "Jan. 2021",
    icon: "fa fa-tasks",
  },
  {
    name: "Typescript It is.",
    description: `My favorite programming language by far. Javascript, but better...`,
    date: "Mar. 2021",
    icon: "fa fa-code",
  },
  {
    name: "Mobile Apps with React Native",
    description: `IOS/Android skills aquired with the creation of React Native`,
    date: "Apr. 2021",
    icon: "fa fa-mobile",
  },
  {
    name: "VinaysReviews",
    description: `An all-in-one review app to rate your favorite books`,
    date: "May 2021",
    icon: "fa fa-star",
  },
  {
    name: "The Creation of SketchDeck",
    description: `A web app inspired by skribbl.io. Have fun while putting your artistic skills to the test.`,
    date: "Jun. 2021",
    icon: "fa fa-paint-brush",
  },
  {
    name: "Trends",
    description: `Putting my mobile development skills to the test with a social media platform.`,
    date: "Oct. 2021",
    icon: "fa fa-share-alt",
  },
  {
    name: "Next.js: The framework in the library",
    description: `React is good, but Next is better...`,
    date: "Nov. 2021",
    icon: "fa fa-rocket",
  },
  {
    name: "Vibes",
    description: `An advanced music player capable of doing it all.`,
    date: "Dec. 2021",
    icon: "fa fa-music",
  },
  {
    name: "The Categories Game",
    description: `The best app I have built so far. A immersive game following the popular trend of wordle`,
    date: "Mar. 2022",
    icon: "fa fa-gamepad",
  },
  {
    name: "GameDay",
    description: `An informative sports news website that has a great user interface. Share your sports takes with other users through our community feed.`,
    date: "Jul. 2022",
    icon: "fa fa-futbol-o",
  },
  {
    name: "DVHS Robotics Novice",
    description: `Novice member in my school's robotics club. I learned how to build and program with VEX.`,
    date: "Sept. 2022-pres.",
    icon: "fa fa-cogs",
  },
  {
    name: "EduTools",
    description: `Built for a hackathon, using AI and API's to help students succeed in school.`,
    date: "Feb. 2023",
    icon: "fa fa-lightbulb-o",
  },
  {
    name: "Team 5776A",
    description: `Drafted to one of DVHS 4 competitive Robotics teams`,
    date: "May 2023-pres.",
    icon: "fa fa-cogs",
  },
  {
    name: "Robotics Summer Camp Mentor",
    description: `Mentored middle schoolers, and taught them how to think and build like engineers.`,
    date: "Summer 2023",
    icon: "fa fa-users",
  },
  {
    name: "Shiva Vishnu Livermore Temple",
    description: `Redesigned organization website (1,000+ weekly visitors), built an Azure NLP AI chatbot, and created a real-time radio streaming feature.`,
    date: "Jun. 2023 - Aug. 2023",
    icon: "fa fa-globe",
  },
  {
    name: "Redevs - Lead Software Engineer",
    description: `Leading a 15-person team building websites for 25+ non-profits across CA, GA, & TX, plus TikTok scraper & analytics dashboards.`,
    date: "Aug. 2024 - Present",
    icon: "fa fa-briefcase",
  },
  {
    name: "VertexStudy LLC",
    description: `Co-founded AI EdTech platform (1,500+ users). Secured $250k valuation at Idea to IPO conference as youngest presenter.`,
    date: "Aug. 2024 - Dec. 2025",
    icon: "fa fa-lightbulb-o",
  },
  {
    name: "EPIC Data Lab - LLM Researcher",
    description: `Researching proxy LLM architecture cost optimization at UC Berkeley, reducing LLM API costs by 35.2%. Presented at EPIC Lab Symposium.`,
    date: "Apr. 2025 - Present",
    icon: "fa fa-flask",
  },
  {
    name: "Practice with Praveena, Inc",
    description: `Programmed full-stack React/Next.js site for 100+ active clients with Vimeo API & developed Roku TV smart TV app.`,
    date: "Jun. 2026 - Aug. 2026",
    icon: "fa fa-tv",
  },
  {
    name: "Trav (iOS App)",
    description: `Designed & deployed native iOS app with Apple Maps API, CoreML AI travel recommendations, and event web scraping pipeline.`,
    date: "Jun. 2026 - Aug. 2026",
    icon: "fa fa-map-marker",
  },
  {
    name: "UC Berkeley",
    description: `Studying EECS at UC Berkeley.`,
    date: "2026 - Present",
    icon: "fa fa-graduation-cap",
  },
];

export default timelineData;
